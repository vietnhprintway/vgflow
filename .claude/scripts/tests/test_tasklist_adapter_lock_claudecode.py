...
def test_codex_runtime_has_own_adapter_lock():
    """When Codex runtime is detected, claude/fallback adapters are rejected."""
    body = _fn_body()
    assert "is_codex_session" in body
    ...
    assert (
        'if is_codex_session and args.adapter in ("claude", "fallback")' in body
    ), "Codex runtime must reject claude/fallback adapters"
