...
1. orchestrator `tasklist-projected --adapter auto` (default) resolves
   from runtime env at runtime — Claude Code => claude, Codex => codex,
   unknown headless => fallback. This eliminates the rationalization knob
...
def test_auto_adapter_resolved_from_env():
    """cmd_tasklist_projected must resolve auto by runtime env."""
    src = ORCH.read_text()
    m = re.search(
    ...
    # Resolves to claude when CLAUDECODE=1, codex for Codex runtime,
    # fallback only for unknown headless runtime.
    auto_idx = body.find('args.adapter == "auto"')
    auto_block = body[auto_idx:auto_idx + 700]
    assert "claude" in auto_block and "codex" in auto_block and "fallback" in auto_block, (
        "Auto-resolve must pick claude, codex, or fallback based on runtime"
    )
    assert "VG_RUNTIME" in body and "CODEX_SESSION_ID" in body, (
        "Codex runtime must be detected explicitly, not folded into fallback"
    )
