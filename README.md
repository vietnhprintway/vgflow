...
**Version:** 2.31.0 · **License:** MIT

**Latest (v2.31.0):** blueprint now owns UI design setup end-to-end: it imports existing mockups, auto-scaffolds missing UI designs, auto-extracts phase design assets, and build hard-gates `<design-ref>` through the shared 2-tier resolver before any executor runs. See [CHANGELOG](CHANGELOG.md).
...
VGFlow không chỉ là happy-path workflow. Từ v2.19.0–v2.31.0, mỗi bug từ production dogfood được trace đến root cause và fix ngay trong cùng pipeline. Dưới đây là những cơ chế bảo vệ được xây từ thực tế:
