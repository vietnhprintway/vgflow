...
| **4. Build** | `/vg:build <N>` (wave-based parallel) | `${PHASE_DIR}/SUMMARY.md` (per-wave commits + per-task evidence), `${PHASE_DIR}/IMPLEMENTATION-NOTES.html` (B87 — AI decision/tradeoff/deviation capture; blueprint emits stub; AI appends `<article>` per decision; close BLOCKs run-complete when OVERRIDE-DEBT or verdict gaps exist without articles) | `/vg:review` reads build output for runtime discovery |
